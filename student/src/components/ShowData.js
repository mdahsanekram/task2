...
class ShowData extends Component
{
  ...
  handlePageClick = (e) => {
    ...
    componentDidMount()
      {         
        axios.post('http://task.atspace.cc/student/ShowData.php')
  ...
  handleEditModel = (userID) =>
  {
    ...
    axios.post('http://task.atspace.cc/student/EditData.php',formData)
    .then((response)=>{
  ...
  handleModalGetData3 = (userID) => {
                ...
                let formData = new FormData();                
                formData.append("userID",userID);
    axios.post('http://task.atspace.cc/student/DeletData.php',formData)
    .then((response)=>{ 
  ...
  submitModal =()=>{
                ...
                // const {name,fname, mob,email,address, city,pin,dob,state, marks}=this.state;

         
          
           axios.post('http://task.atspace.cc/student/UpdateStudent.php',formData)
          .then((response)=>{            
...
handleAddNew=()=>{
  ...
  let formData = new FormData(); 
        ...
        formData.append('pin',this.state.pin)
        formData.append('marks',this.state.marks)
        console.log("ddddname",this.state.name);
        if(this.state.name==""  || this.state.fname==="" || this.state.mob==="" || this.state.email==="" || this.state.address==="" || this.state.city==="" || this.state.pin==="" || this.state.dob==="" || this.state.class == "" || this.state.state==="" || this.state.marks===""){
         alert("please fill all field");
        } else{ 

        axios.post('http://task.atspace.cc/student/AddDataUser.php',formData).then((responce)=>{
          ...
          <Modal size="lg" show={this.state.model1} onHide={()=>this.handleModal()} >            
              ...
              <div className="container">
                ...
                <div className="col-md-6">
                   ...
                   <Select
                   options={option}
                   value={option[this.state.class-5]}
                   onChange={(e)=>this.setState({class:parseInt(e.value)+4})}
                   />
                  </div>
                </div>          
                <div className="col-md-6">
...
<Modal size="lg" show={this.state.model2} onHide={()=>this.setState({model2:false})} >            
    ...
    <div className="container">
      ...
      <div className="col-md-6">
         ...
         <Select
         options={option}
         value={option[this.state.defaultopt]}
         onChange={(e)=>this.setState({class:parseInt(e.value)+4})}
         />
        </div>
      </div>
